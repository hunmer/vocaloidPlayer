<?php
	...
	// error_reporting(-1);
	$s_url = base64_decode(str_replace(' ', '+', $_GET['url']));
	if(strpos($s_url, '//') === 0){
		$s_url = 'http://'.$s_url;
	}

	$ch = curl_init();
	if(strpos($s_url, 'embed.nicovideo.jp') !== false){
		$s_url = "https://www.nicovideo.jp/watch/".getStringByStartAndEnd($s_url,'embed.nicovideo.jp/watch/', '?');
	}else
